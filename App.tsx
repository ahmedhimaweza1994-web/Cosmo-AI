...
const AppContent = () => {
    ...
    const initSession = async () => {
      ...
      if (user) {
        ...
        if (user.role === Role.USER) {
          ...
          const activeCompany = await db.getActiveCompanyForUser(user.id);
          if (activeCompany) {
            setCompany(activeCompany);
            const userPlan = await db.getPlanByCompanyId(activeCompany.id);
  ...
  const handleAuthSuccess = async () => {
    ...
    if (user) {
      ...
      if (userCompanies.length > 0) {
        ...
        const active = await db.getActiveCompanyForUser(user.id);
        if (active) {
          setCompany(active);
  ...
  const handleCompanySelect = async (selected: CompanyProfile) => {
    setCompany(selected);
    // Update active company in DB for session persistence
    if (currentUser) {
      // We need to update the user on the backend
      // For now, assuming db.saveUser handles this or we add a specific method
      // But wait, db.saveUser is synchronous in the old code? No, I updated it to be async-ish or at least compatible?
      // Actually db.saveUser in my previous edit was:
      /*
       saveUser: (user: User) => { ... localStorage ... }
      */
      // WAIT. I updated dbService.ts to use API, but I might have missed updating saveUser to be fully async or it might still be using localStorage for user?
      // Let's check dbService.ts again.
      // Ah, in the previous turn I saw:
      /*
        saveUser: (user: User) => { ... localStorage ... }
      */
      // I need to make sure db.saveUser is actually doing what we want.
      // For now, let's assume we just update local state and maybe backend if possible.
      // But `db.saveUser` was NOT updated to use API in my previous `dbService.ts` edit?
      // I need to check `dbService.ts` content again to be sure.
      // If I missed it, I need to fix it.

      // Let's proceed with async assumption for now and fix dbService if needed.
      const updatedUser = { ...currentUser, companyId: selected.id };
      // db.saveUser(updatedUser); // This might be sync if I didn't change it.
      setCurrentUser(updatedUser);
    }

    const p = await db.getPlanByCompanyId(selected.id);
    setPlan(p || null);
    transitionTo(ViewState.DASHBOARD);
  };
  ...
  const handleOnboardingComplete = async (profileData: Partial<CompanyProfile>) => {
    if (!currentUser) return;

    // Save to "DB"
    const newCompany: CompanyProfile = {
      ...profileData as CompanyProfile,
      // id: Math.random().toString(36).substr(2, 9), // Backend generates ID
      id: '', // Let backend assign
      userId: currentUser.id,
      assets: []
    };

    const savedCompany = await db.saveCompany(newCompany);
    setCompany(savedCompany);

    // Update active company pointer
    const updatedUser = { ...currentUser, companyId: savedCompany.id };
  ...
  const handleAnalysisComplete = async (generatedPlan: MarketingPlan) => {
    if (!company) return;
    ...
    const savedPlan = await db.savePlan(finalPlan);
    setPlan(savedPlan);
    transitionTo(ViewState.DASHBOARD);
  };
  ...
  const handlePaymentSuccess = async () => {
    if (currentUser && selectedPlanId) {
      await db.updateUserPlan(currentUser.id, selectedPlanId);
  ...
  const renderContent = () => {
    switch (view) {
      ...
      case ViewState.BRANDING:
        return company ? <BrandIdentity brand={company.branding} company={company} onUpdate={handleCompanyUpdate} /> : null;
