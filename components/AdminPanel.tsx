...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onNavigate, onLogout }) => {
  ...
  const TabButton = ({ id, icon: Icon, label }: any) => (
    <button 
      ...
      className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all w-full mb-2 ${
        activeTab === id ? 'bg-primary-600 text-white shadow-lg' : 'text-slate-400 hover:bg-white/5 hover:text-white'
      }`}
    >
      <Icon size={18} />
                   ...
                   {['SANDBOX', 'LIVE'].map(m => (
                     <button
                       ...
                       className={`px-4 py-1.5 rounded-md text-sm font-bold transition-all ${
                         settings.payment.mode === m ? 'bg-primary-600 text-white' : 'text-slate-400'
                       }`}
                     >
                       {m}
                     </button>
