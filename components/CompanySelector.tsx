...
export const CompanySelector: React.FC<Props> = ({ user, onSelect, onNavigate, onLogout }) => {
   ...
   const handleSaveNewCompany = async () => {
      ...
      const fullProfile: CompanyProfile = {
         // id: Math.random().toString(36).substr(2, 9), // Let backend assign ID
         id: '',
         userId: user.id,
      ...
      // Save to DB
      try {
         const savedCompany = await db.saveCompany(fullProfile);
         // Trigger Select with the REAL saved company (with ID)
         onSelect(savedCompany);
      } catch (error) {
